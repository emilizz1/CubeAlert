...
static class EditorExploder2DUtils
{
    /// <summary>
    ...
    /// <param name="obj">game object to select</param>
    /// <returns>true if value has been changed</returns>
    public static bool ColorSelection(string label, SerializedProperty value)
    {
        var oldValue = value.colorValue;
        EditorGUILayout.BeginHorizontal();
        value.colorValue = EditorGUILayout.ColorField(label, value.colorValue);
        EditorGUILayout.EndHorizontal();
        return oldValue != value.colorValue;
    }
    ...
    public static bool Vector2(string label, string toolTip, SerializedProperty input)
    {
        var oldValue = input.vector2Value;
        EditorGUILayout.BeginHorizontal();
        GUILayout.Label(new GUIContent(label, toolTip), GUILayout.MaxWidth(150));
        input.vector2Value = EditorGUILayout.Vector2Field("", input.vector2Value);
        EditorGUILayout.EndHorizontal();
        return input.vector2Value != oldValue;
    }
